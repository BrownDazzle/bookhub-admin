...
export const PublisherForm: React.FC<PublisherFormProps> = ({
  ...
  const onSubmit = async (data: PublisherFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/publishers/${params?.publisherId}`, data);
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/publishers/${params?.publisherId}`);
      router.refresh();
      router.push(`/publishers`);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input disabled={loading} placeholder="Publisher value" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
